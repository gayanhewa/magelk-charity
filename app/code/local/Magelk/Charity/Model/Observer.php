...
class Magelk_Charity_Model_Observer
{
    ...
    public function updateQty($observer)
    {
        ...
        foreach ($charity as $key => $row) {
            $charity[$key]["qty"] = $configurables[$key];
            $charity[$key]["total"] = round($charity[$key]["qty"]*$charity[$key]["donation"], 4);
        }

        Mage::getSingleton('core/session')->setCharity($charity);
